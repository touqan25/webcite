...
document.querySelector('#copyButton2').addEventListener('click', async function() {
    ...
    if (selectedText) {
        try {
            await navigator.clipboard.writeText(`"${selectedText}," (Last Name, "${tabTitle}").\n\nLast Name, First Name. ${tabTitle} Website Name. Day Month, Year. ${tabURL} Accessed ${formatMLADate(currentDate)}`);
            console.log("Selected text copied to clipboard:", selectedText);
            } catch (error) {
                console.error("Error copying selected text to clipboard:", error);
            }
